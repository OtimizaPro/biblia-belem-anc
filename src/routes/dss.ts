...
/** Clamp limit/offset to safe positive ranges */
function safeLimit(raw: string | undefined, defaultVal: number, max = 500): number {
...
dss.get('/manuscripts', async (c) => {
  try {
    ...
    const limit = safeLimit(c.req.query('limit'), 100);
